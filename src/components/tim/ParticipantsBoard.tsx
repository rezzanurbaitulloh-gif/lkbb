    ...
    <>
      ...
      >
        {sorted.map((p:any, idx:number)=> {
          ...
          return (
            <article key={p.id} className="w-[270px] shrink-0 snap-start sm:w-[300px]">
              ...
              <Link href={`/dukungan?peleton=${p.slug}`} className="mt-1.5 inline-flex items-center gap-1.5 text-[10px] font-bold tracking-[0.12em] text-[#D9FF3F] hover:underline">
                ✦ DUKUNG →
              </Link>
            </article>
          )
