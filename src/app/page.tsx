...
import { ResultsTeaser } from "@/components/home/HomeExtras"
...
export default async function HomePage(){
  ...
  try {
    ...
    // To make it work for `https://lkbb.my.id?event=lkbb-test2`, we need to get the query from the request's URL, which is available via `headers().get("x-url")` is not set, but we can get it from `hdrs.get("referer")` is also not reliable
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 pb-[72px] md:pb-0">
        ...
        {showFeatured && <Featured peletons={teams} showSementara={showSementara} showFinal={showFinal} siteSettings={siteSettings} />}
        <ResultsTeaser />
