...
// DELETE /api/admin/events?id= — super saja. Event utama (lkbbvote) dilindungi.
// Menghapus domain dari Vercel lebih dulu, lalu baris event (cascade ke data event).
export async function DELETE(req: Request) {
  ...
  if (token && project) {
    ...
    for (const d of (domains || []) as any[]) vercelResults.push({ domain: d.domain, ok: false, error: "VERCEL_TOKEN belum di-set — hapus manual di dashboard Vercel" })
  }
