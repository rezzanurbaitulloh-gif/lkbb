...
export default function Page(){
  ...
  const load = ()=>{ if(!evReady) return; const s=createBrowserSupabase(); let aq: any = s.from("announcements").select("*").order("created_at",{ascending:false})
      if(!evSuper && ownEvents.length>0) aq = aq.in("event_id", ownEvents)
      aq.then(({data}: any)=> setList(data||[])) }
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><h1 className="text-[18px] font-black">Pengumuman</h1></div><div className="flex gap-2">{selected.size>0 && <Button variant="outline" size="sm" className="rounded-full text-red-600 gap-2" onClick={handleBulkDelete}><Trash2 className="h-3.5 w-3.5"/>Hapus {selected.size} dipilih</Button>}<Button size="sm" className="rounded-full" onClick={openAdd}>Tambah Baru</Button></div></div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[520px] max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid gap-3">
            ...
            <div><label className="text-xs font-bold">Isi Pengumuman</label><textarea value={form.content} onChange={e=> setForm({...form, content:e.target.value})} placeholder="Tulis isi pengumuman" className="w-full min-h-[80px] rounded-xl border border-white/[0.08] bg-background px-3 py-2 text-sm" /></div>
          </div>
